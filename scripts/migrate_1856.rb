...
def repair(game, original_actions, actions, broken_action, data, pry_db: false)
  ...
  if broken_action['type'] == 'buy_train'
    old_train = broken_action['train']

    if broken_action['exchange'] == "4'-0"
      broken_action['exchange'] = '4-3'
    end

    case broken_action['train']
    when "2'-0"
      broken_action['train'] = '2-5'
      broken_action['variant'] = '2'
    when "3'-0"
      broken_action['train'] = '3-4'
      broken_action['variant'] = '3'
    when "4'-0"
      broken_action['train'] = '4-3'
      broken_action['variant'] = '4'
    when "5'-0"
      broken_action['train'] = '5-2'
      broken_action['variant'] = '5'
    end

    puts "        patched: updated train value from #{old_train} to #{broken_action['train']}"
    return
  end

  if broken_action['type'] == 'discard_train'
    case broken_action['train']
    when "2'-0"
      broken_action['train'] = '2-5'
    when "3'-0"
    ...
    puts "        discard train fixed"
    return
  end

  if broken_action['type'] == 'run_routes'
    broken_action['routes'].each do |route|
      old_train = route['train']

      case route['train']
      when "2'-0"
        route['train'] = '2-5'
        puts "        patched: updated train route value from #{old_train} to #{route['train']}"
      when "3'-0"
        route['train'] = '3-4'
        puts "        patched: updated train route value from #{old_train} to #{route['train']}"
      when "4'-0"
